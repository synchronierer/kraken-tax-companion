...
function ReviewsPage(){const s=useApi<Page<Row>>("/api/reviews");return <><TaxFeeReviews/><Box sx={{mt:5}}>{s.data?.items.map(item=><Typography key={String(item.id)} variant="caption" sx={{mr:2}}>{reviewGroup(String(item.code))}</Typography>)}<DataTable title="Weitere Prüfungen" state={s} columns={["occurred_at","kind","code","message"]} detailPath={row=>`/api/reviews/${String(row.id)}`}/></Box></>}
